...
@RestController
public class MenteeProfileImgController {

    private final ProfileImgService profileImgService;

    @Autowired
    public MenteeProfileImgController(ProfileImgService profileImgService, ProfileRepository profileRepository) {
    ...
    @GetMapping(
            //image/{id}
            value = "/menteeProfileImg/{id}",
            produces = {MediaType.IMAGE_PNG_VALUE, MediaType.IMAGE_JPEG_VALUE}
    )
    public ResponseEntity<byte[]> getImage(@PathVariable int id) throws IOException {
        MenteeProfileImgDto menteeProfileImgDto = profileImgService.findMenteeProfileImgByFileId(id);
        String absolutePath
                = new File("").getAbsolutePath() + File.separator + File.separator;
        String filePath = menteeProfileImgDto.getFilePath();
